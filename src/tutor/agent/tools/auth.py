...
from .common import agent_tool, resolve_oauth_value
...
def make_login_google_user_tool() -> Callable[..., object]:
    ...
    def login_google_user(
        ...
        lines = [
            "Logowanie Google przygotowane pomyślnie.",
        ...
        if run_browser_auth:
            lines.append("OAuth wykonany (przeglądarka uruchomiona lokalnie).")
        else:
            lines.append("OAuth pominięty (run_browser_auth=False).")
        return "\n".join(lines)
