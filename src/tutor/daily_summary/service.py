...
class DailySummaryService:
    ...
    def build_summary_for_day(self, *, target_date: date) -> DailySummaryResult:
        ...
        raise RuntimeError(
            "build_summary_for_day cannot be called inside an active asyncio loop. "
            "Use await build_summary_for_day_async(...)."
        )
    ...
    async def _build_lesson_summary(
        ...
        if self._progress_callback:
            self._progress_callback(
                f"[bold green]Zakończono analizę notatek:[/bold green] {event.student_name}"
            )
