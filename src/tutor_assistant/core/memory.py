...
class MemoryService:
    ...
    def __init__(self, *, memory_path: Path | None = None) -> None:
        configured_path = os.getenv("TUTOR_AGENT_MEMORY_PATH", "").strip()
        self._memory_path = memory_path or Path(
            configured_path or DEFAULT_MEMORY_FILE_NAME
        )
    ...
    def set(self, *, namespace: str = DEFAULT_MEMORY_NAMESPACE, key: str, value: str) -> None:
        ...
        if not normalized_key:
            raise ValueError("Klucz pamieci nie moze byc pusty.")
        if not normalized_value:
            raise ValueError("Wartosc pamieci nie moze byc pusta.")
