"""Application service for vacation notifications flow."""
...
class VacationNotificationService:
        ...
        for student_name in sorted(grouped):
            ...
            if send_emails and student_email:
                if self._email_provider is None:
                    raise ValueError(
                        "Brak skonfigurowanego email_provider mimo ustawionego send_emails=True."
                    )
                self._email_provider.send_vacation_notice(
                    recipient_email=student_email,
                    subject="Zmiana terminu zajec",
                    body=message,
                )
                email_sent = True
    ...
    def _build_student_message(self, lesson_dates: tuple[date, ...]) -> str:
        ...
        return (
            "Czesc, musimy przelozyc/odwolac nasze zajecia "
            f"z dnia/dni {dates_text} z powodu mojej nieobecnosci. "
            "Mozesz sprawdzic dostepne terminy w moim harmonogramie: "
