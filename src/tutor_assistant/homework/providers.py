"""Providers and integrations for homework management flow."""
...
class GoogleDriveHomeworkProvider:
    def __init__(
        ...
        *,
        ...
        if not self._parent_folder_id:
            raise ValueError(
                "Brak folderu nadrzednego uczniow. Ustaw GOOGLE_DRIVE_PARENT_FOLDER_ID "
                "lub przekaz --drive-parent-folder-id."
            )
        if not self._homework_database_folder_id:
            raise ValueError(
                "Brak folderu bazy zadan domowych. Ustaw GOOGLE_HOMEWORK_DATABASE_FOLDER_ID "
                "lub przekaz --homework-db-folder-id."
            )
    ...
    def list_homework_database_files(self) -> list[HomeworkDatabaseFile]:
        ...
        try:
            ...
            raise RuntimeError(
                "Nie udalo sie pobrac listy plikow z bazy zadan domowych. "
    ...
    def copy_homework_to_student(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                "Nie udalo sie skopiowac zadania domowego do folderu ucznia. "
                f"Szczegoly: {_format_http_error(exc)}"
            ) from exc
...
class BedrockHomeworkMatcher:
    ...
    def _build_prompt(
        *,
        ...
        return (
            "Jestes asystentem nauczyciela matematyki. Na podstawie podsumowania "
            "ostatniej lekcji wybierz jedno najlepsze zadanie domowe z listy. "
            "Mozesz zwrocic tylko nazwe pliku, ktora znajduje sie na liscie. "
            "Jesli nic nie pasuje, zwroc null.\n\n"
            f"Podsumowanie lekcji:\n{notes_summary}\n\n"
            f"Dostepne pliki zadan domowych:\n{available}\n\n"
            "Odpowiedz tylko po polsku i zwroc wylacznie poprawny JSON "
            "bez markdownu i bez dodatkowego tekstu.\n\n"
            "Wymagany format JSON:\n"
            "{\n"
            '  "selected_homework_name": "dokladna nazwa pliku z listy" | null,\n'
